...
if os.name == 'posix':
    FPS = 60
    GRAVITY = (3 * math.pi / 2, 2.49)  # ok on Android
else:
    FPS = 1
...
tt0 = t0
total = 0
textOnScreen = TextOnScreen(screen, 1200, 2450, [])
textOnScreenBounce = TextOnScreen(screen, 1200, 2450, [])
...
while running:
    ...
    if not pause:
        ...
        for particle in my_particles:
            ...
            tt1 = time.time()
            total = tt1 - tt0
            timingText = "Time: " + "{:.1f}".format(total) + " s"
            textLines = [timingText]
            textOnScreen = TextOnScreen(screen, 1200, 120, textLines)
            particle.display()
